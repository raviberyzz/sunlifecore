...
(function(factory) {
  ...
  var datepickerButton = [
      '<a class="datepicker-button slf-input-group-addon" role="button" aria-haspopup="true" tabindex="0" aria-labelledby="datepicker-bn-open-label-CALENDARID">',
  ...
  var datepickerCalendar = [
      ...
      '		<div class="datepicker-month-fast-next pull-right" role="button" aria-labelledby="datepicker-bn-fast-next-label-CALENDARID" tabindex="0"><span class="fa fa-forward"></span></div>',
      '		<div class="datepicker-month-next pull-right" role="button" aria-labelledby="datepicker-bn-next-label-CALENDARID" tabindex="0"><span class="fa fa-chevron-right"></span></div>',
      '		<div class="datepicker-month-fast-prev pull-left" role="button" aria-labelledby="datepicker-bn-fast-prev-label-CALENDARID" tabindex="0"><span class="fa fa-backward"></span></div>',
      '		<div class="datepicker-month-prev pull-left" role="button" aria-labelledby="datepicker-bn-prev-label-CALENDARID" tabindex="0"><span class="fa fa-chevron-left"></span></div>',
      '		<div id="datepicker-month-CALENDARID" class="datepicker-month" tabindex="0" role="heading" aria-level="5" aria-live="assertive" aria-atomic="true" title="Click or press the Enter key or the spacebar to change the month">July 2015</div>',
      '	</div>',
  ...
  var Datepicker = function(target, options) {
      ...
      //var localeStr = document.documentElement.lang.toLowerCase();
  ...
  Datepicker.prototype.showDaysOfPrevMonth = function(offset) {
    ...
    // SLF: changed to use this.$grid, which takes into account disabled days (e.g. weekends)
    if (offset != null) {
      var $cells = this.$grid.find('td.selectable');
      var $day = $cells.eq($cells.length - 1 - offset);
      this.$grid.attr('aria-activedescendant', $day.attr('id'));
      this.selectGridCell($day.attr('id'));
/*
              var numDays = this.getDaysInMonth(this.year, this.month);
...
*/
  ...
  Datepicker.prototype.handleGridKeyDown = function(e) {
          ...
          var $cells = this.$grid.find('td.selectable');
    //var diff = 7 - this.options.daysOfWeekDisabled.length;
          var colCount = this.$grid.find('tbody tr').eq(0).find('td').length - this.options.daysOfWeekDisabled.length;
          ...
          switch (e.keyCode) {
              ...
              // SLF:  if option is true, then the current date cannot be selected
              var dayAllowed = true;
              if (this.options.isCurrentDateDisabled) {
              ...
              case this.keys.down:
                  {
                      ...
                      if (cellIndex < $cells.length) {
                          ...
                          switch (this.gridType) {
                              case 0: // days grid
                                  this.showDaysOfNextMonth(cellIndex - 1);
                                  break;
                              case 1: // months grid
              ...
              case this.keys.pageup:
                  {
                      ...
                      if (ok) {
                          // SLF: reset $cells to new grid; always make go to end of month
              ...
              case this.keys.pagedown:
                  {
                      ...
                      if (ok) {
                          // SLF: reset $cells to new grid; always make go to first of month
                          $cells = this.$grid.find('td.selectable');
                          //if ($('#' + active).attr('id') == undefined) {
  ...
  Datepicker.prototype.show = function() {
    ...
    groupOffsetLeft = 0;  // SLF: always want left aligned to input box, so keep offset as 0

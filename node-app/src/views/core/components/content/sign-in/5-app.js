...
function onProcessPhonesArray() {
    ...
    const phonesArray = ["+1-208-972-3765", "+1-208-912-4324", "+1-208-976-5435", "+1-208-902-6566"];
...
function invokeJourney(journeyId, additionalParams, clientContext) {
    ...
    return new Promise((resolve, reject) => {
        ...
        journeyPlayer.invokePolicy(journeyId, additionalParams, clientContext)
        .then(results => {
            clearTransmitContainer(clientContext);
        ...
        .catch(error => {
            console.error(`error invoking journey with id: ${journeyId}, error: ${error}`);
            clearTransmitContainer(clientContext);
            self.setAppContentApperance(true);
            reject(error);
        })
...
function getTransmitConnectionSettings() {
    if (useMarcServer) {
        const serverUrl = "https://marc.tsdemo.transmit-field.com";
        const appId = "sunlifewebapp";
        const apiTokenId = "";
        const apiToken = "";
        ...
        const appId = "sunlife-example";
        const apiTokenId = "code_examples";
        const apiToken = "2f333a82-d9b8-480e-b6b1-0efebb59a201";
        const realm = "shachar";

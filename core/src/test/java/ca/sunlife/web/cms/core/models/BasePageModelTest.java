...
public class BasePageModelTest {
	...
	public void setInitData() throws LoginException, RepositoryException {
		...
		when(configService.getConfigValues("udoTagsPath", PAGE_PATH)).thenReturn("/content/cq:tags/sunlife/UDO/ca");
		when(configService.getConfigValues(BasePageModelConstants.SITE_URL_CONSTANT, PAGE_PATH)).thenReturn("/content/sunlife/external/ca/en");
		when(request.getRequestPathInfo()).thenReturn(TestUtils.getDummyRequestPathInfo(new String[] { RELEASE_ID }));
		when(configService.getConfigValues("socialMediaImage", PAGE_PATH)).thenReturn("/content/dam/sunlife/images/social.png");
	...
	public void setAltLangParams() throws LoginException, RepositoryException {
		...
		when(configService.getConfigValues(BasePageModelConstants.ALTERNATE_URL_ITEMS_CONSTANT + 0 + "_domain", PAGE_PATH)).thenReturn("https://dev-www.ca.sunlife");
		when(configService.getConfigValues(BasePageModelConstants.ALTERNATE_URL_ITEMS_CONSTANT + 0 + "_languageCode", PAGE_PATH)).thenReturn("en_CA");
		when(configService.getConfigValues(BasePageModelConstants.ALTERNATE_URL_ITEMS_CONSTANT + 0 + "_siteLocation", PAGE_PATH)).thenReturn("/content/sunlife/external/ca/en");
		when(configService.getConfigValues(BasePageModelConstants.ALTERNATE_URL_ITEMS_CONSTANT + 0 + "_defaultLanguage", PAGE_PATH)).thenReturn("true");
		when(configService.getConfigValues(BasePageModelConstants.ALTERNATE_URL_ITEMS_CONSTANT + 1 + "_domain", PAGE_PATH)).thenReturn("https://dev-www.ca.sunlife");
		when(configService.getConfigValues(BasePageModelConstants.ALTERNATE_URL_ITEMS_CONSTANT + 1 + "_languageCode", PAGE_PATH)).thenReturn("fr_CA");
		when(configService.getConfigValues(BasePageModelConstants.ALTERNATE_URL_ITEMS_CONSTANT + 1 + "_siteLocation", PAGE_PATH)).thenReturn("/content/sunlife/external/ca/fr");
	}
	...
	@Test
	public void testInitForCnwPageType() throws LoginException, RepositoryException, IOException, ParseException, ApplicationException, SystemException {
		setInitData();
		...
		basePageModel.setTags(new String[] {"sunlife:UDO/ca/page_audience/Advisor", "sunlife:UDO/ca/abcs", "sunlife:UDO/ca/page_audience/Agent", "sunlife:UDO/ca/page_manager/None", "sunlife:UDO/ca/page_manager/Value1", "sunlife:UDO/ca/page_manager/Value2", "sunlife:UDO/ca/page_dir/None"});
		basePageModel.init();
		
	}

	@Test
	public void testInitForAdvisorPageType() throws LoginException, RepositoryException, IOException, ParseException, ApplicationException, SystemException {
		setInitData();

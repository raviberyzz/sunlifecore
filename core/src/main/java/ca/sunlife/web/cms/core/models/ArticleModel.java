 ...
 */
@Model(adaptables = { SlingHttpServletRequest.class, Resource.class }, defaultInjectionStrategy = DefaultInjectionStrategy.OPTIONAL, adapters = ArticleModel.class, resourceType="sunlife/core/components/content/article")
public class ArticleModel {
	 ...
	 */
	public final void setArticleImage(String articleImage) {
	 ...
	 */
	public final void setOgImage(String ogImage) {
	 ...
	 */
	public final void setOgDescription(String ogDescription) {
	 ...
	 *
	 * @param pageModifiedDate the pageModifiedDate to set
	 */
	public final void setPageModifiedDate(String pageModifiedDate) {
	 ...
	 *
	 * @param publisherLogo the publisherLogo to set
	 */
	public final void setPublisherLogo(String publisherLogo) {
	 ...
	 *
	 * @param ratingRequired the ratingRequired to set
	 */
	public final void setRatingRequired(String ratingRequired) {
	...
	/** The article data. */
	private Map<String,String> articleData = new HashMap<>();
	...
	private void init() {
		...
		try {
			...
			Resource articleResource = resourceResolver.getResource(getFragmentPath().concat(JCR_CONTENT_DATA_MASTER));
			String pagePath = currentPage.getPath();
			if(null != articleResource) {
				logger.debug("Parsing Article Data");
				ValueMap articleContent = articleResource.getValueMap();
				articleData.put(ARTICLE_HEADLINE,articleContent.containsKey(ARTICLE_HEADLINE)?articleContent.get(ARTICLE_HEADLINE,String.class):StringUtils.EMPTY);
				articleData.put(ARTICLE_IMAGE,articleContent.containsKey(ARTICLE_IMAGE)?articleContent.get(ARTICLE_IMAGE,String.class):StringUtils.EMPTY);
				articleData.put(ARTICLE_MAIN_DESCRIPTION,articleContent.containsKey(ARTICLE_MAIN_DESCRIPTION)?articleContent.get(ARTICLE_MAIN_DESCRIPTION,String.class):StringUtils.EMPTY);
				articleData.put(ARTICLE_PAGE_LINK,articleContent.containsKey(ARTICLE_PAGE_LINK)?articleContent.get(ARTICLE_PAGE_LINK,String.class):StringUtils.EMPTY);
				articleData.put(ARTICLE_MINI_DESCRIPTION,articleContent.containsKey(ARTICLE_MINI_DESCRIPTION)?articleContent.get(ARTICLE_MINI_DESCRIPTION,String.class):StringUtils.EMPTY);
				setArticlePublishDate(articleContent);
				setArticleAuthorData(resourceResolver, articleContent);
				setArticleImage(configService.getConfigValues(DOMAIN, pagePath).concat(articleData.get(ARTICLE_IMAGE)));
			}
			...
			setPublisherLogo(configService.getConfigValues(DOMAIN, pagePath).concat(configService.getConfigValues("articlePublisherLogo", pagePath)));
	...
	private void setArticleAuthorData(ResourceResolver resourceResolver, ValueMap articleContent) {
		if(articleContent.containsKey(ARTICLE_AUTHOR)) {
			...
			if(null != authorResource) {
				...
				articleData.put(AUTHOR_NAME,authorContent.containsKey(AUTHOR_NAME)?authorContent.get(AUTHOR_NAME, String.class):StringUtils.EMPTY);
				articleData.put(AUTHOR_BODY,authorContent.containsKey(AUTHOR_BODY)?authorContent.get(AUTHOR_BODY, String.class):StringUtils.EMPTY);
			}else {
